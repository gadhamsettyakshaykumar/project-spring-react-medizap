...
public class CartController {
    ...
    public ResponseEntity<CartModel> addToCart(@RequestParam int medicine_id, @RequestParam int quantity) {
        ...
        try {
            CartModel item = service.addtocart(medicine_id, quantity);
            return ResponseEntity.status(HttpStatus.CREATED).body(item);
        } catch (RuntimeException ex) {
            if (ex.getMessage().contains("Medicine not found")) {
    ...
    public ResponseEntity<CartModel> removeCart(@PathVariable int id) {
        CartModel removed = service.removecartItems(id);
        if (removed == null) {
